...
//
//    void Simulate(std::vector<eAlgorithms> movesSequence, bool ShouldGatherData = true)
//    {
...
public:
    enum eClient
    {
        MCTS = 0, MinMax, Player, Random, None
    };
public:
    GameControler(uint32_t mapSize, uint32_t winCondition);
    DataAnalizer& GetDataStatistics() { return m_Statistics; }

    void AddClient(eClient gameClient);
    void Simulate(uint32_t numberOfSimulations, bool printPreciseMoves);
